...
var checkerLogo1 = setInterval( function(){
    ...
    var nativeAd3 = document.querySelector('#native_network_ad_1');

    if(nativeAd3){
        if(nativeAd3.classList.contains("--nologo")){
            console.log("No Logo");
            clearInterval(checkerLogo1);
        }
        else{
            if(nativeAd3.firstChild && nativeAd3.firstChild.id.indexOf('yandex_rtb') <= -1){
                nativeAd3.setAttribute("style" , "margin-top: 16px; margin-bottom: 20px; display: flex; justify-content: center;")

                var nativeLogoTop = document.createElement("a");
                nativeLogoTop.className = 'nativeLogoTop'
                nativeLogoTop.href = 'https://native.weprodigi.com/';
                nativeLogoTop.target = '_blank';
                nativeLogoTop.innerHTML = '<img src="https://ads.caramel.am/new_logo_svg/advertising.svg" id="top_svg_advertising">';
                nativeAd3.firstChild.setAttribute("style" , "min-width: 300px; min-height: 250px; max-height: 500px; position: relative;")
                nativeAd3.firstChild.appendChild(nativeLogoTop);



                var nativeLogoBottom = document.createElement("a");
                nativeLogoBottom.className = 'nativeLogoBottom';
                nativeLogoBottom.href = 'https://native.weprodigi.com/';
                nativeLogoBottom.target = '_blank';
                nativeLogoBottom.innerHTML = '<div class="bottomBox logoAnimation" id="bottomBox"> <img src="https://ads.caramel.am/new_logo_svg/logo.svg" id="logo_svg_text" > <img src="https://ads.caramel.am/new_logo_svg/caramel.svg" id="logo_svg"> </div>';
                nativeAd3.firstChild.appendChild(nativeLogoBottom);



                var rotate = document.getElementById("bottomBox");
                var logoSvg = document.getElementById("logo_svg");




                rotate.addEventListener('mouseover', function() {
                    logoSvg.className = 'over';
                }, false);
                ...
                var doc = document;
                var newScript = doc.createElement('style');
                var content = doc.createTextNode(`.nativeLogoTop { right: 0; top: -18px; display: flex; align-items: center; width: 300px; height: 18px; position: absolute; margin-top: 0px;} #top_svg_advertising { margin: unset !important; max-width: unset !important; width: 300px !important; height: 5px !important; right: 0px !important; position: absolute !important;} .nativeLogoBottom{width: 145px; height: 20px; position: absolute; bottom: -20px; right: 0px; overflow: hidden; z-index: 2; display: flex; align-items: center;} .bottomBox {width: 100%; height: 100%; display: flex; align-items: center; position: absolute; right: 0px; animation-name: logoBoxAnimation;} .logoAnimation {animation-duration: 1s; animation-iteration-count: 1; animation-timing-function: ease-in-out;} @keyframes logoBoxAnimation{0% { right: -145px; } 100% { right: 0px; }} #logo_svg_text{width: 125px !important; height: 18px !important; right: 0; position: absolute; margin: 0 !important; object-fit: contain;} #logo_svg{width: 15px !important; height: 15px !important; right: 130px; position: absolute; margin: 0 !important;} .over { animation-name: rotate1; animation-duration: 1.5s; animation-iteration-count: infinite; animation-timing-function: linear; } @keyframes rotate1 { from {transform: rotate(0deg);} to {transform: rotate(360deg);} } .out { animation-name: rotate2; animation-duration: 1.5s; animation-iteration-count: 1; animation-timing-function: ease-out; } @keyframes rotate2 { from {transform: rotate(0deg);} to {transform: rotate(360deg);} }`);
                newScript.appendChild(content);
                var bodyClass = doc.getElementsByTagName('head')[0];
                bodyClass.appendChild(newScript);
                }
...
var checkerLogo2 = setInterval( function(){
    ...
    var nativeAd3 = document.querySelector('#native_network_ad_2');
    ...
    if(nativeAd3){
        if(nativeAd3.classList.contains("--nologo")){
            ...
            if(nativeAd3.firstChild && nativeAd3.firstChild.id.indexOf('yandex_rtb') <= -1){
                nativeAd3.setAttribute("style" , "margin-top: 16px; margin-bottom: 20px;")
